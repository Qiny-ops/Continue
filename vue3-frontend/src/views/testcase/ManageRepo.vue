  ...
  <div class="manage-page">
    ...
    <div class="page-content">
      ...
      <div v-else-if="list.length === 0" class="empty-box">
        ...
        <p>暂无用例库</p>
        <el-button type="primary" @click="openDialog()">新建用例库</el-button>
      </div>

      <div v-else class="data-list">
        <div class="list-head">
          <span class="col-name">名称</span>
          <span class="col-count">版本数</span>
        ...
        <div class="list-body">
          <div v-for="item in list" :key="item.id" class="list-row">
            ...
            <span class="col-count">{{ item.versions_count || 0 }}</span>
    ...
    <el-dialog v-model="dialogVisible" :title="editing ? '编辑用例库' : '新建用例库'" width="440px" destroy-on-close>
      <el-form :model="form" label-width="70px">
        <el-form-item label="名称" required>
          <el-input v-model="form.name" placeholder="请输入用例库名称" />
        </el-form-item>
...
import { Plus, ArrowLeft, FolderOpened } from '@element-plus/icons-vue'
import { repositoryApi } from '@/api/modules/testcase'
...
const fetchData = async () => {
  ...
  loading.value = true
  try {
    const res = await repositoryApi.getRepositories({ project: projectId.value })
...
const setDefault = async (item) => {
  try {
    await repositoryApi.updateRepository(item.id, { is_default: true })
...
const deleteItem = async (item) => {
  try {
    ...
    await repositoryApi.deleteRepository(item.id)
...
const submit = async () => {
  ...
  try {
    if (editing.value) {
      await repositoryApi.updateRepository(editing.value.id, { name: form.name.trim(), description: form.description })
