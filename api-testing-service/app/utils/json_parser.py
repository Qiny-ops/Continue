...
多策略重试解析：
- <thought>/<thinking> 标签处理
- markdown 代码块提取
- 类型自动转换（dict <-> list）

旧版别名 extract_json_block / extract_json_object / extract_json_array 保留，
供 engine.py / executor.py 等调用方使用。
"""
import json
import logging
import re
...
# 旧版别名（兼容 engine.py / executor.py 等调用方）
